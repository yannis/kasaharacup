...
class TeamPoolStandings
  ...
  # builder and the waiting area both seed from.
  #
  # `clear_unranked: true` additionally CLEARS a rank the standings no longer
  # support: results do get taken back (a point entered on the wrong tablet, a
  # bout re-run), and a rank left behind went on putting the former leader of an
  # unfinished pool into the tree on every rebuild.
  #
  # Opt-in for the same reason as PoolStandings' twin: this pool ranks nobody
  # both when its results were taken back and when it never had any. The one
  # caller today (Encounter#recompute_pool_standings!) only runs off an
  # encounter completing or un-completing, so it asks; a generation-time caller
  # added here later must not, or the tie-break ranks this class's own contract
  # invites the admin to type would be wiped by an event that recorded nothing.
  def self.persist_ranks!(teams:, encounters:, clear_unranked: false)
