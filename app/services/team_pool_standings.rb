...
class TeamPoolStandings
  Row = Data.define(:team, :team_wins, :team_losses, :team_draws,
    :individual_wins, :individual_losses, :individual_draws,
  ...
  CASCADE_KEY = ->(r) {
    [-r.team_wins, r.team_losses, -r.team_draws,
      -r.individual_wins, r.individual_losses, -r.individual_draws,
  ...
  private def compute_row(team)
    ...
    Row.new(team: team, team_wins: tw, team_losses: tl, team_draws: td,
      individual_wins: iw, individual_losses: il, individual_draws: idr,
      points_scored: ps, points_conceded: pc, rank: nil, tied: false)
  end

  private def completed_for(team)
    encounters.select do |encounter|
      [encounter.team_1_id, encounter.team_2_id].include?(team.id) && encounter.result.complete?
    end
  end

  private def no_data?(rows)
    rows.all? { |r| r.team_wins.zero? && r.team_losses.zero? && r.team_draws.zero? }
  end
