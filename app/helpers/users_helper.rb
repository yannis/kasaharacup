module UsersHelper
  ...
  def user_admin_links(user, nav: nil)
    links = []
    links << link_to("<span class='glyphicon glyphicon-plus'></span> #{t("users.show.register_yourself")}".html_safe, new_cup_user_kenshi_path(@current_cup, current_user, self: true), class: "btn btn-xs btn-info") if user == current_user && !user.registered_for_cup?(@cup)
