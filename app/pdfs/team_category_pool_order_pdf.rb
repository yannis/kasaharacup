...
class TeamCategoryPoolOrderPdf < Prawn::Document
  ...
  # The gap between one pool's ties and the next.
  SPACER_HEIGHT = 10
  ...
  # One table per pool, a small gap between two pools, and a pool that would
  # not fit under the last one starts a new page rather than being split
  # across two: a pool's ties are read together. The column headers head every
  # page.
  private def draw_order_table(ties)
    ...
    ties.chunk_while { |a, b| a.pool_number == b.pool_number }.each do |pool_ties|
      pool_table = order_table(pool_ties.map { |tie| tie_row(tie) })
      if !first_on_page && pool_table.height + SPACER_HEIGHT > cursor
        start_new_page
        draw_column_headers
        first_on_page = true
      end
      move_down SPACER_HEIGHT unless first_on_page
      pool_table.draw
      first_on_page = false
    end
  end

  private def draw_column_headers
    headers = [I18n.t("pool_fight_order.number"), I18n.t("pool_fight_order.pool"),
      I18n.t("pool_fight_order.white"), nil, I18n.t("pool_fight_order.red")]
    order_table([headers]) do
  ...
  private def tie_row(tie)
    [tie.order.to_s, tie.label, tie.encounter.team_1&.poster_name, "x", tie.encounter.team_2&.poster_name]
  end

  # The header row and every pool share these columns, so they line up as one
  # list. The columns add up to 522pt, just inside the 523pt the default
  # margins leave.
  private def order_table(data, &block)
    make_table(data, cell_style: {inline_format: true, size: 12}) do
      cells.padding = 6
