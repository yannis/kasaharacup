...
class NavigationDropdownComponent < ViewComponent::Base
  def initialize(name:, text:, links:, css_class: nil, position: :right)
    @name = name.underscore
    @text = text
    @links = links
    @position_class = (position == :left) ? "left-0" : "right-0"
    @css_class = css_class.presence ||
      "focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-offset-red-100 focus:ring-brand
