...
describe "A participation" do
  context "without team_id an individual_category_id" do
    it {expect{create :participation}.to raise_error(ActiveRecord::RecordInvalid, "Validation failed: Individual category can't be blank")}
    context "when an individual_category_id is set" do
      let(:participation) {build :participation, individual_category: mock_model(IndividualCategory)}
