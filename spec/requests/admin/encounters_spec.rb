...
RSpec.describe "Admin encounters" do
  ...
  describe "POST team_swap" do
    ...
    it "answers a turbo_stream that redraws the tree for the acting admin" do
      first, second = round_one
      moving_in = second.team_1
      ...
      expect(response.body).to include(%(target="encounter_tree_team_category_#{bracket_only.id}"))
      expect(response.body).to include(moving_in.name)
      expect(first.reload.team_1).to eq moving_in
    end

    it "answers 422 with a message when a drag swap is invalid" do
      first, = round_one
      ...
      expect(response).to have_http_status(:unprocessable_content)
      expect(response.parsed_body["message"]).to include("already in this encounter")
    end

    it "answers 422 when the client's expected_team_id is stale" do
      first, second = round_one

      post admin_team_category_encounter_team_swap_path(bracket_only, first),
        params: {slot: 1, team_id: second.team_1_id, expected_team_id: second.team_2_id},
        headers: turbo_headers
