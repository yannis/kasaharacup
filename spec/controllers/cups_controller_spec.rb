...
RSpec.describe CupsController, type: :controller do
  let!(:cup1) { create :kendocup_cup, start_on: Date.current + 1.month, events: [create(:kendocup_event)] }
  ...
  context "When not logged in" do
    describe "when GET to :show for cup1," do
      before { get :show, params: {id: Date.current.year, locale: I18n.locale} }

      it { expect(response).to be_success }
      it { expect(assigns(:cup)).not_to be_nil }
