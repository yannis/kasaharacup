...
RSpec.describe EncounterLineupSeeder do
  ...
  it "seeds both sides from their previous order without confirming them" do
    a = members(t1, 3)
    b = members(t2, 3)
    ...
    expect(fresh.reload.lineup_1_set?).to be false
    expect(fresh.lineup_2_set?).to be false
  end
  ...
  it "leaves an already-populated (but unconfirmed) side untouched" do
    a = members(t1, 3)
    members(t2, 3)
    fresh = create(:encounter, team_category: tc, team_1: t1, team_2: t2)
    described_class.new(fresh).call # first seed lays down roster order
    reordered = a.reverse

    # Simulate a manual reorder that hasn't been re-seeded; a second call is a no-op.
    EncounterLineup.new(fresh).seed(t1, reordered.map(&:id))
    described_class.new(fresh).call
