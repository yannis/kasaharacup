...
RSpec.describe TeamCategoryPoolOrderPdf, :en do
  ...
  it "lists the ties in fighting order, one pool after the other" do
    ...
    expect(texts.grep(/\APool /)).to eq [
      "Pool 1 — 1/3", "Pool 1 — 2/3", "Pool 1 — 3/3",
      "Pool 2 — 1/3", "Pool 2 — 2/3", "Pool 2 — 3/3"
    ]
  end

  it "leaves a gap between one pool's ties and the next" do
    pool_of(1, "Alpha", "Bravo", "Charlie")
    pool_of(2, "Delta", "Echo", "Foxtrot")
    PoolEncounterGenerator.new(category).call

    baselines = text_positions_in(described_class.new(category))
      .filter_map { |baseline, text| baseline if text.start_with?("Pool ") }
      .sort.reverse
    steps = baselines.each_cons(2).map { |upper, lower| upper - lower }

    # Five steps down six rows: the one between the pools, the third, is wider
    # by the spacer row. The others differ at most by a rounded point.
    gap, *rows = steps.values_at(2, 0, 1, 3, 4)
    expect(gap).to be >= rows.max + described_class::SPACER_HEIGHT - 1
  end
  ...
  # Each page is one uncompressed content stream; the logo's image stream holds
  # no text runs, so it drops out of the pages that name a pool.
  def pools_by_page(pdf)
    pdf.render.scan(/stream(.*?)endstream/m).flatten.filter_map do |stream|
      pools = stream.scan(/\[(.*?)\]\s*TJ/m).flatten.map { |run| pdf_text_of(run) }
        .filter_map { |text| text[/\APool (\d+) —/, 1]&.to_i }
      pools.uniq unless pools.empty?
    end
  end

  it "starts a new page before a pool rather than splitting it across two" do
    10.times { |number| pool_of(number + 1, *%w[Alpha Bravo Charlie Delta].map { |name| "#{name} #{number}" }) }
    PoolEncounterGenerator.new(category).call

    pdf = described_class.new(category)
    pages = pools_by_page(pdf)

    expect(pdf.page_count).to eq 2
    expect(pages.flatten).to eq (1..10).to_a
    expect(texts_in(pdf).count("Red")).to eq 2
  end
